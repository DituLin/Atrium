...
| S-002 | Go module and entrypoint | S-001 | `go.mod` (module `github.com/DituLin/Atritum`, Go 1.25), `cmd/atrium/main.go`, `internal/cli` with `version`, `serve`, `init` stubs (cobra), `internal/app` lifecycle (signal handling, graceful shutdown) | `go build ./...`; `atrium version` prints version + commit (ldflags) |
